...
def clean_text_sentiment_analysis(reviews, spacy_conf="fr_core_news_md"):
    ...
    for review in reviews:
        ...
        text = re.sub(r'<span.*?>.*?</span>', "", review)
        ...
        text = " ".join(            #Remove unused elements for nlp such as mails, urls, ponctuations, etc...
            [
                token.lemma_
                for token in doc
        ...
        for (
            ent
        ) in doc.ents:  # Remove named entities using spaCy: Names, Surnames, Places
